...
void attachInterruptWithDebounce(uint32_t pin, voidFuncPtr callback, uint32_t mode, uint32_t debounceTime)
{
	...
	if (pin_desc->pin_current_function != pin_desc->pin_mux_aon_sel_i2c) {

		ret = hal_pinmux_set_function(pin_desc->pin_no, pin_desc->pin_mux_aon_sel_i2c);
		if (ret == 0)
			pin_desc->pin_current_function = pin_desc->pin_mux_aon_sel_i2c;
		else
			goto FAIL;
	}

	hal_eint_mask(pin_get_eint_num(pin_desc));
	...
	ret = hal_eint_register_callback(pin_get_eint_num(pin_desc), callback, NULL);
...
FAIL:
	 ...
	 * return false;
	 */
	hw_uart_printf("attachInterruptWithDebounce err!!!\n");
	return ;
}
...
void detachInterrupt(uint32_t pin)
{
	...
	hal_eint_unmask(pin_get_eint_num(pin_desc));
...
FAIL:
	 ...
	 * return false;
	 */
	hw_uart_printf("detachInterrupt err!!!\n");
	return ;
}
